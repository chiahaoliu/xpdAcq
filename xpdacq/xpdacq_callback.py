""" module for callback used in xpdAcq"""
import os
import uuid
import time
import yaml
...
DARK_MD_FIELD = 'sc_dk_field_uid'

class Calibration(CallbackBase):
    """callback to perform calibration after a scan

    Parameters
    ----------
    db : databroker.Broker instance, optional
        an object serves as an interface to integrate different databases.
        default is locally configured, read-only one
    image_field : str, optional
        key for detector image field in database. default is "pe1_image"
    calibrant : str, optional
        calibrant which is going to be used. it can be full path to the
        customized calibrant file(with ".D" extention) or it can one of
        the predefined calibrant instances.
        list of valid instance is:
        ['CeO2', 'mock', 'Ni', 'Au', 'Al', 'CuO', 'LaB6_SRM660b',
         'LaB6_SRM660c', 'Si', 'Si_SRM640e', 'TiO2', 'ZnO', 'LaB6',
         'Si_SRM640a', 'CrOx', 'quartz', 'cristobaltite', 'Cr2O3',
         'Si_SRM640d', 'C14H30O', 'LaB6_SRM660a', 'Si_SRM640b',
         'Si_SRM640', 'alpha_Al2O3', 'AgBh', 'NaCl', 'Si_SRM640c',
         'PBBA']
    wavelength : flot, optional
        current of x-ray wavelength in angstrom. default to value read
        out from current xpdacq.Beamtime object
    calibration_collection_uid : str, optional
        uid of calibration collection. default is auto-generated ones.
    param_file_name : str, optional
        file name of yaml file that stores calibration parameters.
        default is "pyFAI_<MMDD-HHMM>.yml"
    ...
    kwargs :
        extra keyword arguments pass to calibration method.
    """
    def __init__(self, db=None, image_field=None, calibrant=None,
                 wavelength=None, detector=None, calib_collection_uid=None,
                 param_file_name=None, , **kwargs):
        if db is None:
            from databroker.broker import Databroker as db
        self.db = db
        ...
        if detector is None:
            detector = Perkin()
        # setup calibration instance
        calib = Calibration(calibrant=calibrant)
        calib.detector = detector

        self.calib = calib

    def start(self, doc):
        """method applied to start document"""
        dark_uid = doc.get(dark)
